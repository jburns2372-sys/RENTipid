...
describe("SOC Authorization Service Matrix", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    prismaMock.userMfa.findUnique.mockResolvedValue({ status: 'ENABLED', last_verified_at: new Date() });
  ...
  it("AUTHZ-P1-001 - Unauthenticated user denied", async () => {
    ...
    expect(logAdministrationEvent).toHaveBeenCalledWith(expect.objectContaining({
      action: "ADMIN_AUTHORIZATION_DENIED",
      metadata: expect.objectContaining({ denial_reason: expect.stringContaining("SOC_ACCESS_DENIED_UNAUTHENTICATED") }),
    }));
  });
  ...
  it("AUTHZ-P1-017 & AUTHZ-P1-018 - Deleted/Missing User or Expired session denied", async () => {
    ...
    expect(logAdministrationEvent).toHaveBeenCalledWith(expect.objectContaining({
      action: "ADMIN_AUTHORIZATION_DENIED",
      metadata: expect.objectContaining({ denial_reason: expect.stringContaining("SOC_ACCESS_DENIED_USER_NOT_FOUND") }),
    }));
  });
  ...
  const generateRoleTest = (role: string, status: string, expectedDenialReason: string | null) => {
    return async () => {
      ...
      if (expectedDenialReason) {
        ...
        expect(logAdministrationEvent).toHaveBeenCalledWith(expect.objectContaining({
          action: "ADMIN_AUTHORIZATION_DENIED",
          metadata: expect.objectContaining({ denial_reason: expect.stringContaining(expectedDenialReason) }),
        }));
      } else {
  ...
  it("AUTHZ-P1-009 - Unauthorized Admin denied (Phase 1 specific)", generateRoleTest("Admin", "Verified", "SOC_ACCESS_DENIED_PERMISSION"));
  it("AUTHZ-P1-010 - Finance Admin limited", generateRoleTest("Finance Admin", "Verified", "SOC_ACCESS_DENIED_ROLE"));
  it("AUTHZ-P1-011 - Compliance Admin limited", generateRoleTest("Compliance Admin", "Verified", "SOC_ACCESS_DENIED_PERMISSION"));
  ...
  it("AUTHZ-P1-024 & AUTHZ-P1-025 - Database status/role overrides stale JWT", async () => {
    ...
    (getServerSession as jest.Mock).mockResolvedValue({ user: { id: id1, role: "Super Admin", status: "Verified", iat: Date.now() / 1000 + 10000 } });

    // DB says Renter / Verified
    prismaMock.user.findUnique.mockResolvedValueOnce({ id: id1, role: "Renter", status: "Verified", updated_at: new Date() });
    await expect(requireSecurityPermission(SECURITY_PERMISSIONS.DASHBOARD_VIEW)).rejects.toThrow('NEXT_REDIRECT');
    expect(logAdministrationEvent).toHaveBeenCalledWith(expect.objectContaining({
      action: "ADMIN_AUTHORIZATION_DENIED",
      metadata: expect.objectContaining({ denial_reason: expect.stringContaining("SOC_ACCESS_DENIED_ROLE") }),
    }));
    ...
    (getServerSession as jest.Mock).mockResolvedValue({ user: { id: id2, role: "Super Admin", status: "Verified", iat: Date.now() / 1000 + 10000 } });
    // DB says Super Admin / Suspended
    prismaMock.user.findUnique.mockResolvedValueOnce({ id: id2, role: "Super Admin", status: "Suspended", updated_at: new Date() });
    await expect(requireSecurityPermission(SECURITY_PERMISSIONS.DASHBOARD_VIEW)).rejects.toThrow('NEXT_REDIRECT');
    expect(logAdministrationEvent).toHaveBeenCalledWith(expect.objectContaining({
      action: "ADMIN_AUTHORIZATION_DENIED",
      metadata: expect.objectContaining({ denial_reason: expect.stringContaining("SOC_ACCESS_DENIED_ACCOUNT_STATUS") }),
    }));
  });
  ...
  it("AUTHZ-P1-023 - AuditLog failure does not grant access", async () => {
    const id = Date.now().toString() + Math.random().toString();
    (getServerSession as jest.Mock).mockResolvedValue({ user: { id, iat: Date.now() / 1000 + 10000 } });
    prismaMock.user.findUnique.mockResolvedValue({ id, role: "Renter", status: "Verified", updated_at: new Date() });

    // Simulate audit failure
    (logAdministrationEvent as jest.Mock).mockRejectedValue(new Error("Audit DB Down"));
