...
| UAICS-DH-DEC-005 | Production Remediation | Knowledge Intent Routing | Generic fallback & broad section chunks | Bind `listing.create.how_to` strictly to `provider-workflow-status:workflow-status-guidance-listings` and map `"What items are prohibited on RENTipid"` aliases to `listing.item.restriction` (`POLICY_TAXONOMY`) | Resolves Owner answer-quality defects in production, eliminating booking contamination and enumerating 25 prohibited item policy items. |
