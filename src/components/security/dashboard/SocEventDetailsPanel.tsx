...
export function SocEventDetailsPanel({ event, onClose }: { event: SocCommandCenterEvent | null, onClose: () => void }) {
  if (!event) {
    return (
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl flex flex-col shadow-lg h-[400px] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 text-sm">
        <div className="grid grid-cols-2 gap-4">
            <DetailItem label="Event Code" value={event.eventCode} icon={<Activity className="w-3 h-3" />} />
            <DetailItem label="Classification" value={event.classification} icon={<Shield className="w-3 h-3" />} />
            <DetailItem label="Severity" value={event.severity} />
            <DetailItem label="Environment" value={event.environment} />
            <DetailItem label="Lifecycle" value={event.lifecycle} />
            <DetailItem label="Timestamp" value={new Date(event.occurredAt).toLocaleString()} />
            <DetailItem label="Source" value={event.sourceType} icon={<Database className="w-3 h-3" />} />
            <DetailItem label="Verified Location" value={event.locationClassification === 'UNKNOWN' ? 'Location Unknown' : event.city ? `${event.city}, ${event.country}` : 'Location Unknown'} />
            <DetailItem label="Target Resource" value={event.targetReference ?? "Not available"} icon={<Server className="w-3 h-3" />} />
            <DetailItem label="Processing Status" value={event.processingStatus} />
        </div>
        ...
        {event.incident && (
            <div className="bg-purple-900/20 border border-purple-500/20 p-3 rounded-lg mt-4">
                ...
                <div className="text-slate-300 font-mono text-sm">{event.incident.reference}</div>
                <div className="text-xs text-slate-400 mt-1">Status: {event.incident.status}</div>
            </div>
        )}
      </div>
    </div>
