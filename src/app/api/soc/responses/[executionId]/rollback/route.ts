...
export async function POST(req: Request, { params }: { params: { executionId: string } }) {
  try {
    ...
    const hasPermission = await assertSecurityPermissionForService(user.id, SECURITY_PERMISSIONS.RESPONSE_ROLLBACK);
    if (!hasPermission) {
    ...
    const execution = await rollbackSecurityResponse(user.id, params.executionId);
