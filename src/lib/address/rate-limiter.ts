...
export class DatabaseRateLimiter {
  static async consume(key: string, limit: number, windowMs: number): Promise<boolean> {
    try {
      if (Math.random() < 0.01) {
...
export class AddressRateLimiter {
  ...
  static async consumeAutocomplete(ip: string, userId: string): Promise<boolean> {
    ...
    return DatabaseRateLimiter.consume(key, this.MAX_AUTOCOMPLETE_PER_MINUTE, 60000);
  }

  static async consumeDetails(ip: string, userId: string): Promise<boolean> {
    const key = `rl:addr:det:${userId}:${ip}`;
    return DatabaseRateLimiter.consume(key, this.MAX_DETAILS_PER_MINUTE, 60000);
  }
}
