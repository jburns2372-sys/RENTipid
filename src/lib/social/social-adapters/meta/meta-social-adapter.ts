...
export class MetaSocialAdapter implements SocialAdapter {
  platformId: SocialPlatform = SOCIAL_PLATFORMS.FACEBOOK;

  getCapabilities(): ProviderCapability[] {
    return [
      'accountConnection', 'accountValidation', 'healthCheck', 
      'publishText', 'publishImage', 'publishVideo', 'scheduledPublishing',
      'analytics', 'comments', 'feedback', 'webhookEvents'
    ];
  }

  async validateConnection(accountId: string): Promise<boolean> {
    // External credentials NOT REQUIRED for Phase 4 closure.
    return false;
  }
