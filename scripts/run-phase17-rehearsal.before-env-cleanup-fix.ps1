...
param(
    ...
    [switch]$StopAfterPackageVerification
)
...
function Clear-PgPassword {
    if (Test-Path Env:\PGPASSWORD) { Remove-Item Env:\PGPASSWORD -Force }
}
...
function Expand-AndVerifyPackage {
    ...
    $DetachedChecksum = $DetachedChecksumPath
    $Zip = $ZipPath
    $ExpectedHash = ((Get-Content $DetachedChecksum -Raw).Trim() -split '\s+')[0]
    $ActualHash = (Get-FileHash $Zip -Algorithm SHA256).Hash

    if ($ExpectedHash -ine $ActualHash) {
        throw "PHASE17 ZIP CHECKSUM FAILED."
    }

    $PackageVerified = $true
    Write-Host "Package checksum verified: True"
    ...
    [PSCustomObject]@{
        ...
        ZipHash = $ActualHash.ToLowerInvariant()
...
$PackageRoot = Join-Path $env:TEMP ("phase17-package-" + [guid]::NewGuid())
$ExtractedPackage = Join-Path $PackageRoot "package"
$evidenceDirectory = Join-Path `
    $repositoryRoot `
    'phase17-rehearsal-evidence-work'
$summary = [ordered]@{
    server = $Server
...
$currentStagePath = "$zipPath; $detachedChecksumPath; $ExtractedPackage"
...
try {
    ...
    New-Item -ItemType Directory -Path $ExtractedPackage | Out-Null

    $package = Expand-AndVerifyPackage `
        -ZipPath $zipPath `
        -DetachedChecksumPath $detachedChecksumPath `
        -Destination $ExtractedPackage

    if (-not $package.PackageVerified) {
        throw 'Package verification returned an unverified result.'
    }

    $summary.package_verified = $true

    if ($VerifyPackageOnly -or $StopAfterPackageVerification) {
        $summary.execution_result = 'PHASE17_PACKAGE_STAGE_PASSED'
        $exitCode = 0
    }
    else {
        ...
        if (Test-Path -LiteralPath $evidenceDirectory) {
            throw "Evidence work directory already exists: $evidenceDirectory"
        }

        New-Item -ItemType Directory -Path $evidenceDirectory | Out-Null
        @(
            "detached_zip_sha256=$($package.ZipHash)"
            "zip_entry_count=$($package.EntryCount)"
            "internal_checksums_verified=$($package.InternalChecksumCount)"
            'unauthorized_entries=0'
        ) | Set-Content `
            -LiteralPath (Join-Path $evidenceDirectory 'package-validation.txt') `
            -Encoding UTF8

        $currentStagePath = 'psql.exe'
    ...
    $currentStagePath = $preValidationPath
    ...
    $currentStagePath = $remediationPath
    ...
    $currentStagePath = $postVerificationPath
    ...
    $currentStagePath = $diagnosticPath
    ...
    Write-Host "Failing file/path: $currentStagePath"
}
finally {
    Clear-PgPassword
    if (Test-Path Env:\PGSSLMODE) { Remove-Item Env:\PGSSLMODE -Force }
    ...
    $temporaryFullPath = [IO.Path]::GetFullPath($PackageRoot)
